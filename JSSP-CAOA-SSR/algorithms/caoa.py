# =============================================================================
# FILE: CAOA.py
# DESKRIPSI: Crocodile Ambush Optimization Algorithm (CAOA) Standar.
# =============================================================================
import time
import numpy as np
...
def CAOA(N, max_iter, lb, ub, dim, fobj, 
    ...
    # 2. LOOP UTAMA ITERASI
    for t in range(max_iter):
        old_positions = pos.copy()
        old_fitness = fitness.copy()
        n_depleted_count = 0
        
        # A. Seleksi Pemimpin Stokastik (Berdasarkan probabilitas dari nilai fitness)
        ...
        # B. Pembaruan Posisi (Pergerakan Buaya)
        for i in range(N):
        ...
        # D. Peluruhan Energi Adaptif (Semakin jauh bergerak, semakin cepat energi habis)
        distances = np.sqrt(np.sum((pos - old_positions)**2, axis=1))
        energies = energies - gamma * distances
        
        # E. Mekanisme Pemulihan Energi (Buaya kelelahan beristirahat di posisi acak baru)
        depleted = energies <= 0
        if np.any(depleted):
